...
class BaseLinesChartController: BaseChartController {
    ...
    func updateChartRangeTitle(animated: Bool) {
        ...
        let toDate: Date
        if isZoomed {
            fromDate = Date(timeIntervalSince1970: TimeInterval(zoomedChartRange.lowerBound) + .hour)
            toDate = Date(timeIntervalSince1970: TimeInterval(zoomedChartRange.upperBound))
        } else {
            fromDate = Date(timeIntervalSince1970: TimeInterval(initialChartRange.lowerBound) + .hour)
            toDate = Date(timeIntervalSince1970: TimeInterval(initialChartRange.upperBound))
        }
